...
func get_function(txt string, start int) (int, string) {
	for i := start; i < len(txt) - 2; i++ {
		if txt[i:i+3] == "mul" {
			return i+3, "mul"
		} else if i < len(txt) - 4 && txt[i:i+5] == "don't" {
			return i+5, "don't"
		} else if txt[i:i+2] == "do" {
			return i+2, "do"
		}
	}
	return len(txt), ""
...
func parse(txt string) []Operation {
	...
	for i := 0; i < len(txt); i++ {
		var name string
		i, name = get_function(txt, i)
		fmt.Println(name)
		if i > len(txt) - 6 {
			break
		}
		if name == "do" && txt[i] == '(' && txt[i + 1] == ')' {
			do = true
			continue
		} else if name == "don't" && txt[i] == '(' && txt[i + 1] == ')' {
			do = false
			continue
		}
